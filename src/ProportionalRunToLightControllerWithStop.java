...
public class ProportionalRunToLightControllerWithStop extends DifferentialWheels {
    //setup values
    ...
    public ProportionalRunToLightControllerWithStop () {
        super();
    ...
    private void calculateSpeed() {
        //Matrix lightMatrix = _lightControllerMatrix.times(_lightSensorValueMatrix);
       // Matrix distanceMatrix = _distanceControllerMatrix.times(_distanceSensorValueMatrix);
        Matrix difference = _lightSensorValueMatrix.minus(_distanceSensorValueMatrix);
        Matrix speedMatrix = _lightControllerMatrix.times(difference);

        System.out.println(speedMatrix.get(0,0) + " " +  speedMatrix.get(1,0));

        setSpeed(speedMatrix.get(0, 0), speedMatrix.get(1, 0));
    }
    ...
    private void setPriorities() {
        ...
        _distanceControllerMatrix = new Matrix(new double[][] {
                //left, middle left, front left, right, middle right, front right
                { 0.5, 0.05, 0.8, 0, 0, 0},
                { 0, 0, 0, 0.5, 0.05, 0.8}
        });
    ...
    private void getValues() {
        ...
        _distanceSensorValueMatrix = new Matrix(new double[][]{
    ...
    public static void main(String[] args) {
        ProportionalRunToLightControllerWithStop controller = new ProportionalRunToLightControllerWithStop();
        controller.run();
    }
}